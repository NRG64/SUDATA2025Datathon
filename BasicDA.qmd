...
```{r basic summary and cleaning}

df_clean <- df_fe %>%
  mutate(
    date = as.Date(date),
    delivery_time_deviation = as.numeric(delivery_time_deviation),
    high_risk_fraction      = as.numeric(high_risk_fraction),
    transport_costs         = as.numeric(transport_costs),
    infrastructure_costs    = as.numeric(infrastructure_costs),
    risk_costs              = as.numeric(risk_costs),
...
ggplot(df_fe, aes(x = factor(high_risk_fraction_round), y = delivery_time_deviation)) +
